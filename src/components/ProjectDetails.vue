      ...
      <el-table-column label="操作">
        <template #default="scope">
          <el-button type="warning"
                     plain
                     style="width: 100%"
                     @click="handleEdit(scope.row)">重新分配
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    page(newPage) {
      let _this = this
      axios.get("http://localhost:8888/limsProjects/listNotNull/" + newPage + "/" + this.pageSize).then(function (resp) {
  ...
  created() {
    let _this = this
    axios.get("http://localhost:8888/limsProjects/listNotNull/1/" + this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data.data
      for (let elem of _this.tableData) {
        elem.assignTime = elem.assignTime.replace("T", " ")
      }
